...
{
    internal abstract class KafkaConsumer<K, V> : IHostedService
    {
        protected readonly ILogger<KafkaConsumer<K, V>> Logger;
        protected readonly ConsumerConfig ConsumerConfig;
        protected readonly string Topic;

        protected IConsumer<K, MessageBase<V>> Consumer;

        public KafkaConsumer(ILogger<KafkaConsumer<K, V>> logger,
            ...
            Task.Run(() =>
            {
                while (cancellationToken.IsCancellationRequested == false)
                {
                    try
                    {
                        ...
                        Logger.LogInformation($"Message: {System.Text.Json.JsonSerializer.Serialize(consumer.Message.Value)} received from {consumer.TopicPartitionOffset.Topic}");
